...
public class ProductRepositoryWithBagRelationshipsImpl implements ProductRepositoryWithBagRelationships {
    ...
    public Optional<Product> fetchBagRelationships(Optional<Product> product) {
        return product.map(this::fetchWishLists);
    }
    ...
    public List<Product> fetchBagRelationships(List<Product> products) {
        return Optional.of(products).map(this::fetchWishLists).orElse(Collections.emptyList());
    }

    Product fetchWishLists(Product result) {
        return entityManager
            .createQuery("select product from Product product left join fetch product.wishLists where product.id = :id", Product.class)
            .setParameter("id", result.getId())
            .getSingleResult();
    }

    List<Product> fetchWishLists(List<Product> products) {
        ...
        List<Product> result = entityManager
            .createQuery("select product from Product product left join fetch product.wishLists where product in :products", Product.class)
